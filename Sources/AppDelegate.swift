...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func createNewWindow() {
        ...
        BridgeController.shared.register(tabManager: tabManager)

        let contentView = ElectrobunShellView(tabManager: tabManager)
        let window = NSWindow(
    ...
    private func buildMainMenu() -> NSMenu {
        ...
        // App menu
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "About termo", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Quit termo", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
...
extension AppDelegate: NSWindowDelegate {
    func windowWillClose(_ notification: Notification) {
        ...
        if let manager = tabManagers.first(where: { $0.window === window }) {
            BridgeController.shared.unregister(tabManager: manager)
        }
